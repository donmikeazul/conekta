...
class ConektaController(http.Controller):
    ...
    def create_params(self, acquirer):
        ...
        if request.session['uid'] is not None:
            ...
            create_date = mktime(datetime.strptime(create_at, '%Y-%m-%d %H:%M:%S').timetuple())
            write_at = so.partner_id.write_date
            updated_date = mktime(datetime.strptime(write_at, '%Y-%m-%d %H:%M:%S').timetuple())
        ...
        shipping_address['street1'] = so.partner_shipping_id.street
